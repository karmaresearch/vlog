...
void Loader::sortAndInsert(int permutation, int nindices, bool inputSorted,
                           ...
                           Inserter *ins, const bool aggregated, const bool storeRaw,
    ...
    while (!merger.isEmpty()) {
        ...
        if (t.o != po || t.p != pp || t.s != ps) {
            count++;
            ins->insert(permutation, t.s, t.p, t.o, t.count, posWriter, treeWriter,
                        aggregated);
            ps = t.s;
            pp = t.p;
            po = t.o;
    ...
    ins->flush(permutation, posWriter, treeWriter, aggregated);
...
void Loader::exportFiles(string tripleDir, string* dictFiles,
    ...
    //Export the triples
    std::ofstream file(outputFileTriple, ios_base::binary);
    {
        boost::iostreams::filtering_ostream out;
        out.push(boost::iostreams::gzip_compressor());
        out.push(file);
        char tmpString[1024];
        boost::filesystem::directory_iterator iterator(tripleDir);
        for (; iterator != boost::filesystem::directory_iterator(); ++iterator) {
            BOOST_LOG_TRIVIAL(debug) << "Exporting file " << iterator->path();
            LZ4Reader reader(iterator->path().c_str());
            reader.parseByte(); //Is quad? Ignored for now...
            while (!reader.isEof()) {
                sprintf(tmpString, "%ld %ld %ld", reader.parseLong(), reader.parseLong(), reader.parseLong());
                out << tmpString << endl;
            }
        }
    }
...
void Loader::load(bool onlyCompress, string triplesInputDir, string kbDir,
    ...
    if (parallelThreads > ncores) {
        BOOST_LOG_TRIVIAL(debug) << "The parallelThreads parameter is above the number pf physical cores. I set it to " << ncores;
        parallelThreads = ncores;
    }

    if (maxReadingThreads > parallelThreads) {
        BOOST_LOG_TRIVIAL(debug) << "I cannot read with more threads than the available ones. I set it to = " << parallelThreads;
        maxReadingThreads = parallelThreads;
    }

    if (dictionaries > parallelThreads) {
        BOOST_LOG_TRIVIAL(debug) << "The dictionary partitions cannot be higher than the maximum number of threads. I set it to = " << parallelThreads;
    ...
    if (onlyCompress) {
        ...
        string outputTriples = kbDir + string("/triples.gz");
        string outputDict = kbDir + string("/dict.gz");
        exportFiles(permDirs[0], fileNameDictionaries,
                    dictionaries, outputTriples, outputDict);
        for (int i = 0; i < nperms; ++i)
    ...
    loadKB(kb, kbDir, true, dictionaries, dictMethod, nindices, aggrIndices, sample,
           sampleRate, storePlainList, fileNameDictionaries, nperms, signaturePerm, permDirs,
           totalCount);
...
void Loader::loadKB(KB &kb, string kbDir, bool storeDicts, int dictionaries, string dictMethod,
                    int nindices, bool aggrIndices, bool sample, double sampleRate, bool storePlainList,
                    string *fileNameDictionaries, int nperms, int signaturePerms,
    ...
    if (nindices == 1) {
        ...
        if (!aggrIndices) {
            t2 = boost::thread(
                     boost::bind(&Loader::sortAndInsert, this,
                                 2, nperms, false, permDirs[2],
                                 (string*)NULL,
                                 treeWriters[2], ins, false));
            if (nindices == 4) {
                t3 = boost::thread(
                         boost::bind(&Loader::sortAndInsert, this,
                                     3, nperms, false, permDirs[3],
                                     (string*)NULL,
        ...
        if (aggrIndices) {
            if (nindices == 4) {
                t2 = boost::thread(
                         boost::bind(&Loader::sortAndInsert, this,
                                     3, nperms, false, aggr2Dir,
                                     (string*)NULL,
                                     treeWriters[3], ins, true));
            }
            sortAndInsert(2, nperms, false, aggr1Dir, NULL, treeWriters[2], ins, true);
            if (nindices == 4) {
                t2.join();
            }
        }
        ...
        if (!aggrIndices) {
            ts[0] = boost::thread(
                        boost::bind(&Loader::sortAndInsert, this,
                                    1, nperms, false, permDirs[1], (string*) NULL,
                                    treeWriters[1], ins, false));
            ts[1] = boost::thread(
                        boost::bind(&Loader::sortAndInsert, this,
                                    3, nperms, false, permDirs[3], (string*)NULL,
                                    treeWriters[3], ins, false));
            ts[2] = boost::thread(
                        boost::bind(&Loader::sortAndInsert, this,
                                    4, nperms, false, permDirs[4], (string*)NULL,
                                    treeWriters[4], ins, false));

            //Start two more threads
            at1 = boost::thread(boost::bind(&Loader::sortAndInsert, this, 2, nperms,
                                            false, permDirs[2], (string*) NULL,
                                            treeWriters[2], ins, false));
            at2 = boost::thread(boost::bind(&Loader::sortAndInsert, this, 5, nperms,
                                            false, permDirs[5], (string*) NULL,
                                            treeWriters[5], ins, false));

        } else {
            ts[0] = boost::thread(
                        boost::bind(&Loader::sortAndInsert, this,
                                    1, nperms, false, permDirs[1], &aggr1Dir,
                                    treeWriters[1], ins, false));
            ts[1] = boost::thread(
                        boost::bind(&Loader::sortAndInsert, this,
                                    3, nperms, false, permDirs[2], (string*)NULL,
                                    treeWriters[3], ins, false));
            ts[2] = boost::thread(
                        boost::bind(&Loader::sortAndInsert, this,
                                    4, nperms, false, permDirs[3], (string*)NULL,
                                    treeWriters[4], ins, false));

        }

        sortAndInsert(0, nperms, false, permDirs[0], aggrIndices ? &aggr2Dir : NULL,
                      treeWriters[0], ins, false, storePlainList, sampleWriter, sampleRate);
        for (int i = 0; i < 3; ++i) {
            ts[i].join();
        }
        at1.join();
        ...
        if (aggrIndices) {
            ...
            t[0] = boost::thread(boost::bind(&Loader::sortAndInsert, this, 2, nperms,
                                             false, aggr1Dir, (string*) NULL,
                                             treeWriters[2], ins, true));
            t[1] = boost::thread(boost::bind(&Loader::sortAndInsert, this, 5, nperms,
                                             false, aggr2Dir, (string*) NULL,
                                             treeWriters[5], ins, true));
            t[0].join();
            t[1].join();
        }
    }
    ...
    if (sample) {
        ...
        loadKB(kb, sampleKB, false, dictionaries, dictMethod, nindices, aggrIndices,
               false, sampleRate, storePlainList, fileNameDictionaries, nperms,
...
TermCoordinates *CoordinatesMerger::get(nTerm &key) {
    ...
    if (!spoFinished && (opsFinished || elspo.key <= elops.key)
        ...
        if (!posFinished && elpos.key == elspo.key) {
            ...
            if (ncoordinates == 6 || ncoordinates == 4) {
