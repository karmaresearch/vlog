...
#ifdef LINEAGE
    std::vector<LineageInfo> info;
    cardTable = evaluator.evaluateQuery(QSQR_EST, &rootQuery, posBindings, valueBindings, true, info);
#else
    cardTable = evaluator.evaluateQuery(QSQR_EST, &rootQuery, posBindings, valueBindings, true);
#endif
...
TupleIterator *Reasoner::getTopDownIterator(Literal &query,
    ...
    BOOST_LOG_TRIVIAL(debug) << "Get topdown iterator for pattern " << query.tostring(&program, &edb);
    /*VTuple t(3);*/
    std::vector<uint8_t> newPosJoins;
    if (posJoins != NULL) {
        newPosJoins = *posJoins;
