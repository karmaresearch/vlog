...
void RuleExecutionPlan::calculateJoinsCoordinates(const Literal &headLiteral) {
    ...
    for (uint8_t i = 0; i < plan.size(); ++i) {
        ...
        if (i == plan.size() - 1) {
            ...
            for (uint8_t j = 0; j < existingVariables.size(); ++j) {
                ...
                if (isVarNeeded) {
                    ...
                    // BOOST_LOG_TRIVIAL(debug) << "Adding variable " << (int) existingVariables[j];
        ...
        // BOOST_LOG_TRIVIAL(debug) << "currentLiteral = " << currentLiteral->tostring(NULL, NULL);
        for (uint8_t x = 0; x < currentLiteral->getTupleSize(); ++x) {
            ...
            if (t.isVariable()) {
                ...
                // BOOST_LOG_TRIVIAL(debug) <<  "Considering variable " << (int) t.getId() << ", litVars = " << (int) litVars << ", found = " << found;
                if (found) {
                    ...
                    if (isVariableNeeded) {
                        if (i == plan.size() - 1) {
                            ...
                            if (isNew) {
                                ...
                                // BOOST_LOG_TRIVIAL(debug) << "Adding new variable " << (int) t.getId();
                            }
                        }
                    }
                }
...
bool RuleExecutionPlan::hasCartesian() {
    for (int i = 1; i < joinCoordinates.size(); i++) {
        BOOST_LOG_TRIVIAL(debug) << "joinCoordinates[" << i << "]: size = " << joinCoordinates[i].size();
...
SemiNaiver::SemiNaiver(std::vector<Rule> ruleset, EDBLayer &layer,
    ...
    opt_intersect(opt_intersect), opt_filtering(opt_filtering), running(false) {
    ...
    for (std::vector<Rule>::iterator itr = ruleset.begin(); itr != ruleset.end();
            ++itr) {
        RuleExecutionDetails d(*itr, ruleid++);
    ...
    while (positionCards.size() > 0) {
        ...
        for (int i = 0; i < positionCards.size(); ++i) {
            ...
            if (plan.plan[positionCards[i].first]
                ...
                // BOOST_LOG_TRIVIAL(debug) << "Shared var: " << (int) plan.plan[positionCards[i].first]->getSharedVars(vars)[0];
                found = true;
...
//#endif
        ...
        while (optimalOrderIdx < nBodyLiterals) {
            ...
            if (max == 1)
                max = ruleDetails.lastExecution - 1;
            BOOST_LOG_TRIVIAL(debug) << "Evaluating atom " << optimalOrderIdx <<
                                     " min=" << min << " max=" << max;
            ...
            if (lastLiteral && !endTable->isEmpty()) {
                ...
                if (block.iteration == iteration && (listDerivations.size() == 0 ||
                                                     listDerivations.back().iteration
                                                     != iteration)) {
                    listDerivations.push_back(block);
                }
            }

            if (!lastLiteral && (currentResults == NULL ||
...
//#endif
    ...
    if (endTable->isEmpty(iteration)) {
        ...
        stats.derivation = listDerivations.back().table->getNRows();
    }
