                    ...
                    for (size_t j = 0; j < body.size(); ++j) {
                        ...
                        if (literal.getPredicate().getType() == EDB) {
                            for (uint8_t m = 0; m < literal.getTupleSize(); ++m) {
                                Term term = literal.getTermAtPos(m);
...
bool RuleExecutor::isUnifiable(const Term_t * const value, const size_t sizeTuple,
    ...
    for (size_t i = 0; i < sizeTuple; ++i) {
        ...
        Term t = adornedRule.getHead().getTermAtPos(pos);
        if (!t.isVariable()) {
            ...
            if (nCurrentJoins > 0) {
                ...
                for (int i = 0; i < bindings.size(); i += nCurrentJoins) {
                    ...
                    for (int j = 0; j < nCurrentJoins; ++j) {
                        t.set(Term(0, bindings[i + j]), posJoinsLiteral[j]);
                    }
                    QSQQuery query(Literal(l.getPredicate(), t));
...
void RuleExecutor::copyLastRelInAnswers(QSQR *qsqr,
    ...
    if (nTuples > 0) {
        ...
        for (uint8_t i = 0; i < adornedRule.getHead().getTupleSize(); ++i) {
            Term t = adornedRule.getHead().getTermAtPos(i);
            if (t.isVariable()) {
                posVars[nvars++] = i;
            } else {
                tuple[i] = t.getValue();
