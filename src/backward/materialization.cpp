...
bool Materialization::cardIsTooLarge(const Literal &lit, Program &p,
    ...
    for (auto &rule : p.getAllRules()) {
        if (rule.getNIDBPredicates() == 0 && rule.getBody().size() == 1) {
            ...
            if (nsubs != -1) {
                QSQQuery query(rule.getBody()[0].substitutes(subs, nsubs));
                size_t card = layer.estimateCardinality(&query);
                BOOST_LOG_TRIVIAL(debug) << "Card for literal " << query.getLiteral()->tostring(NULL, NULL) << " is " << card;
                if (card > 10000) {
                    BOOST_LOG_TRIVIAL(debug) << "Query " << lit.tostring(NULL, NULL) << " is ignored";
                    return true;
                }
            }
        }
...
void Materialization::guessLiteralsFromRules(Program &p, DictMgmt *dict,
        EDBLayer &layer) {
    //Potential literals
...
bool Materialization::execMatQuery(Literal &l, bool timeout, EDBLayer &kb,
    ...
    bool failed = false;
    QSQQuery q(l);
    BOOST_LOG_TRIVIAL(debug) << "Getting query " << l.tostring(&p, dict);
    ...
    if (!timeout || timeoutSeconds == 0) {
        ...
        failed = !evaluateQueryThreadedVersion(&kb, &p, dict, &q,
                                               &output,
                                               timeoutSeconds);
    }
    ...
    if (failed) {
        BOOST_LOG_TRIVIAL(debug) << "Query " << l.tostring(&p, dict) <<
                                 " failed after " <<
                                 sec.count() * 1000 << " ms";
    } else { // Not failed
...
void Materialization::getAndStorePrematerialization(EDBLayer & kb, Program & p,
        DictMgmt * dict, bool timeout, long timeoutSeconds) {
    int predIdx = 0;
    ...
    try {
        for (std::vector<Literal>::iterator itr =  prematerializedLiterals.begin();
            ...
            bool failed = execMatQuery(*itr, timeout, kb, p, dict, predIdx,
                                       timeoutSeconds);
            if (!failed) {
        ...
        while (repeatPrematerialization && failedQueries.size() > 0) {
            ...
            for (auto &el : failedQueries) {
                bool failed = execMatQuery(el, timeout, kb, p, dict, predIdx,
                                           timeoutSeconds);
                if (!failed) {
        ...
        for (auto &rule : p.getAllRules()) {
            BOOST_LOG_TRIVIAL(debug) << "Rule: " << rule.tostring(&p, dict);
        }
    } catch (int v) {
...
void Materialization::rewriteLiteralInProgram(Literal & prematLiteral, Literal & rewrittenLiteral, EDBLayer & kb, Program & p) {
    ...
    Predicate kbPredicate = kb.getKBPredicate();
