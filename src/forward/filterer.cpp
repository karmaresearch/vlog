    ...
    for (const auto el : bodyLiterals) {
        ...
        if (!foundSmall && el.getPredicate().getId() != headRule.getPredicate().getId()) {
            ...
            if (estimate < 10000000) { //I must be quick to query...
                ...
                BOOST_LOG_TRIVIAL(debug) << "Card of "<< el.tostring(naiver->getProgram(), naiver->getDict()) << estimate << "<- too large";
            }
        }
        i++;
    }
...
bool TableFilterer::producedDerivationInPreviousStepsWithSubs(
    ...
    // posLiteral[0].second thus is the number of the variable in the currentQuery,
    // so we need to use posVarsInLit[posLiteral[0].second] to get its position in 
    // currentQuery.
...
bool TableFilterer::producedDerivationInPreviousStepsWithSubs_rec(
    ...
    if (rLit == NULL) {
        ...
        Term tAtCurQuery = currentQuery.getTermAtPos(posLit_second);
        ...
        Term tAtOutQuery = outputQuery.getTermAtPos(posHead_first);
    ...
    if (nrLit != NULL) {
        ...
        for (int j = 0; j < nrLit->getTupleSize(); ++j) {
            Term t = nrLit->getTermAtPos(j);
            if (t.isVariable()) {
                for (int i = 0; i < rLit->getTupleSize(); ++i) {
                    Term t2 = rLit->getTermAtPos(i);
                ...
                for (int i = 0; i < headBlockRule.getTupleSize(); ++i) {
                    Term t2 = headBlockRule.getTermAtPos(i);
                    if (t2.isVariable() && t.getId() == t2.getId()) {
                        if (foundHead)
                            throw 10;
    ...
    for (const auto &el : mapSubstitutions) {
        ...
        Tuple t = currentQuery.getTuple();
        t.set(Term(0, el.first), posLit_second);
        ...
        if (nrLit != NULL) {
            ...
            if (mapSubstitutionsInBlockQuery.count(k)) {
                ...
                for (auto posH : possibleHeads) {
                    if (!localBindings.count(posH)) {
                        ...
                        Tuple t = outputQuery.getTuple();
                        t.set(Term(0, posH), joinRandNRLits.second);
                        Literal newHead(outputQuery.getPredicate(), t);

                        //Create the body
                        Tuple t2 = srLit.getTuple();
                        t2.set(Term(0, posH), joinRandNRLits.second);
            ...
            t.set(Term(0, *possibleHeads.begin()), posHead_first);
            Literal newHead(outputQuery.getPredicate(), t);
            if (!(newHead == srLit)) {
                response = false;
                break;
