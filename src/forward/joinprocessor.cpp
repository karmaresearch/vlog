...
void JoinExecutor::joinTwoToOne(
    ...
    std::shared_ptr<const Column> firstColumn = intermediateResults->getColumn(
    ...
    while (!tableItr.isEmpty()) {
        ...
        //are joining. In this case, I can only copy the output field
        ...
        //BOOST_LOG_TRIVIAL(debug) << "Counter1=" << counter1 << " Counter2=" << counter2 << " Cout=" << cout << "IdenticalJoin=" << identicalJoin;
...
void JoinExecutor::verificativeJoinOneColumnSameOutput(
    ...
    if (tableItr.getNTables() == 1) {
        ...
        // BOOST_LOG_TRIVIAL(debug) << "tableItr.getNTables() != 1";
        //First collect all the results and then do a global merge
        ...
        while (!tableItr.isEmpty()) {
            ...
            uint8_t columns[1];
            columns[0] = plan.joinCoordinates[currentLiteral][0].second;
            std::vector<std::shared_ptr<Column>> cols = itr->getColumn(1,
                                              columns);
            secondColumnCreator.concatenate(cols[0].get());
...
void JoinExecutor::join(SemiNaiver * naiver, const FCInternalTable * t1,
    ...
    if (JoinExecutor::isJoinVerificative(t1, plan, currentLiteral)) {
        ...
        if (JoinExecutor::isJoinTwoToOneJoin(plan, currentLiteral)) {
            ...
            //keys are few. Otherwise, merge join.
            if (t1->estimateNRows() <= THRESHOLD_HASHJOIN && joinsCoordinates.size() < 3) {
...
bool JoinExecutor::isJoinSelective(JoinHashMap & map, const Literal & literal,
    ...
    for (JoinHashMap::iterator itr = map.begin(); itr != map.end(); ++itr) {
        ...
        tuple.set(Term(0, itr->first), joinPos);
    ...
    }
    {
        ...
        DoubleJoinHashMap::const_iterator itr2;
        {
            ...
            while (ok) {
                ...
                if (njoinfields < 2) {
                    if (njoinfields > 0)
                        tuple.set(Term(0, itr1->first), idxJoinFieldInLiteral1);
                    start = itr1->second.first;
                    end = itr1->second.second;
                } else {
                    tuple.set(Term(0, itr2->first.first), idxJoinFieldInLiteral1);
                    tuple.set(Term(0, itr2->first.second), idxJoinFieldInLiteral2);
                ...
                while (outputLiteral != NULL && start < end) {
                    Tuple t = outputLiteral->getTuple();
                    for (uint8_t i = 0; i < nPosFromFirst; ++i) {
                        t.set(Term(0, values[start + posFromFirst[i].second]), posFromFirst[i].first);
                    }
...
void JoinExecutor::mergejoin(const FCInternalTable * t1, SemiNaiver * naiver,
    ...
    if (idxColumnsLowCardInMap.size() == 0) {
        ...
        while (idxs.size() > 0) {
            ...
            Tuple t = literalToQuery.getTuple();
            ...
            for (uint32_t j = 0; j < idxColumnsLowCardInLiteral.size(); ++j) {
                ...
                t.set(Term(0, bagValuesColumns[j][idxs[j]]), idxInLiteral);
...
void JoinExecutor::do_mergejoin(const FCInternalTable * filteredT1,
    ...
    for (auto t2 : tables2) {
        ...
        // Pity that we have to sort filtered1 for each itr2 entry.
