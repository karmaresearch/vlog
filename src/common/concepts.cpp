...
std::string Literal::tostring(Program *program, DictMgmt *dict) const {
...
int Literal::mgu(Substitution *substitutions, const Literal &l, const Literal &m) {
    ...
    for (int i = 0; i < l.getTupleSize(); ++i) {
        Term tl = l.getTermAtPos(i);
        Term tm = m.getTermAtPos(i);
...
int Literal::getSubstitutionsA2B(Substitution *substitutions,
    ...
    for (int i = 0; i < a.getTupleSize(); ++i) {
        Term ta = a.getTermAtPos(i);
        Term tb = b.getTermAtPos(i);
...
Literal Literal::substitutes(Substitution * subs, const int nsubs) const {
    Tuple newTuple((uint8_t) this->tuple.getSize());
...
std::string Rule::tostring(Program * program, DictMgmt * dict) const {
    std::string output = std::string("HEAD=") + head.tostring(program, dict) + std::string(" BODY= ");
    for (int i = 0; i < body.size(); ++i) {
        output += body[i].tostring(program, dict) + std::string(" ");
    }
    return output;
}
...
Literal Program::parseLiteral(std::string l) {
    ...
    while (tuple.size() > 0) {
        ...
        if (std::isupper(term.at(0))) {
            t.push_back(Term((uint8_t) dictVariables.getOrAdd(term), 0));
        } else {
            //Constant
            term = rewriteRDFOWLConstants(term);
            nTerm dictTerm;
            if (!kb->getDictMgmt()->getNumber(term.c_str(), term.size(), &dictTerm)) {
                //BOOST_LOG_TRIVIAL(error) << "This should not happen. Every schema terms should be found in main memory. Term " << term << " was not found";
                //throw 10;
            ...
            t.push_back(Term(0, dictTerm));
        }
    }

    Tuple t1((uint8_t) t.size());
    int pos = 0;
    for (std::vector<Term>::iterator itr = t.begin(); itr != t.end(); ++itr) {
        t1.set(*itr, pos++);
    }
